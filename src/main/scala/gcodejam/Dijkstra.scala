...
object Dijkstra extends CodeJamApp[Int] {
  ...
  class Minus(private[Minus] val elem: Quat) extends Quat
  case object Minus {
    ...
    def unapply(e: Quat): Option[Quat] = e match {
      case m: Minus => Some(m.elem)
      case _ => None
    }
  }
  ...
  def minimalPrefixRepeat(quats: Seq[Quat], aim: Quat): Option[Int] = {
    ...
    (Stream.iterate[Quat](One)(whole * _) take 8 map
      ...
      Stream.from(0, quats.length) map { case (prefix, add) => prefix map (add + _) }
      ).flatten.headOption
  }
